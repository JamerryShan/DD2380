...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # It is recommended you use the np.random module
        sch = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon = sch.value(current_total_steps)
        if np.random.rand() < epsilon:
    ...
    def q_learning(self):
        ...
        # ADD YOUR CODE SNIPPET BETWEENEX. 2.3
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q, s_current, self.action_list, steps)
                # print(action)
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                ...
                Q[s_current, action] = Q_old[s_current, action] + self.alpha * (R + self.gamma * np.nanmax(Q[s_next]) - Q_old[s_current, action])
                ...
                steps += 1
                if R == self.settings.rewards[0] and R_total > bestR:
                    bestR = R_total
                    Q_best = Q
                    break
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        value = self.initial_p + (self.final_p - self.initial_p) * (t / self.schedule_timesteps)
        return value
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2